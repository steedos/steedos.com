...
使用华炎魔方仪表盘，可以连接任意数据库，在界面上编写查询语句，然后再配置页面和图表，就可以轻松实现各种类型的仪表盘报表，比如`Boxplot，Chart - Line, Bar, Area, Pie, Scatter`等，本教程将描述如何在华炎魔方中把它配置出来。
...
在“设置”应用中，进入“统计分析→页面”界面可以维护华炎魔方所有的页面记录，页面记录详细界面就是一个仪表盘设计器。
...
我们可以进入”设置“应用，然后在”统计分析→页面“界面新建一个页面，保存后我们将在页面详细界面上看到redash页面设计器。
