...
\
> 不要使用此函数进行空比较，如 MyDateTime__c == null。而应使用[ISBLANK](/docs/admin/functions#isblank)。
...
■ 如果您希望使用“日期”字段，请使用[TODAY](/docs/admin/functions#today)函数。
...
■ 如果您希望使用日期时间字段，请参阅[NOW](/docs/admin/functions#now)函数。
...
> * 如果字段没有值，则使用 BLANKVALUE 函数返回指定置换表达式对应的值；如果您只想检查字段是否有值，则使用[ISBLANK](/docs/admin/functions#isblank)函数。
...
■ 如果字段没有值，则使用 [BLANKVALUE](/docs/admin/functions#blankvalue) 函数返回指定表达式；如果您只想检查字段是否有值，则使用 ISBLANK 函数。
...
:::info
■ 该函数只支持多选的选项列表字段，要使用单选选项列表字段请使用 [ISPICKVAL](/docs/admin/functions#ispickval)。

■ 可以使用 [ISBLANK](/docs/admin/functions#isblank) 决定选项列表字段是否为空。
...
:::info
■ 该函数只支持单选的选项列表字段，要使用多选选项列表字段请使用 [INCLUDES](/docs/admin/functions#includes)。
