...
对象列表组件会监听`@data.changed.project`事件，并根据传入的参数判断是否刷新对象列表组件。
...
在华炎魔方对象列表页面中**编辑**并保存记录后，会刷新对象列表组件，它是通过在弹出的编辑表单中监听提交成功事件执行此动作来实现的，
...
在华炎魔方对象列表页面中**新建**记录后，会跳转到记录详细页，它是通过在弹出的编辑表单中监听提交成功事件执行此动作来实现的，
详情请参考 [对象表单组件源码](https://github.com/steedos/steedos-widgets/blob/1.2/packages/%40steedos-widgets/amis-lib/src/lib/converter/amis/index.js#L350)。  
...
只读模式下，对象表单组件会监听`@data.changed.project`事件，并根据传入的参数判断是否刷新对象表单组件。

要刷新对象表单组件，需在`data`中配置以下参数：
- objectName: `String`。要触发广播事件对象的对象Api名称，即对象列表绑定的对象api名称。
- __deletedRecord: `Boolean`。是否为删除操作，若为真，则跳转到对象列表。选填。

在华炎魔方记录详情页中编辑记录后，会刷新对象表单组件，通过在弹出的编辑表单中监听提交成功事件执行此动作来实现的。[对象表单组件源码](https://github.com/steedos/steedos-widgets/blob/1.2/packages/%40steedos-widgets/amis-lib/src/lib/converter/amis/index.js#L350)。

##### **跳转对象列表组件**

以下示例演示点击按钮后，跳转到名为`project`到对象列表组件。
...
对象表单组件会监听广播，根据传入的参数判断是否跳转对象列表组件。

要跳转对象列表组件，需在`data`中配置以下参数：
- objectName: `String`。要触发广播事件对象的对象Api名称，即对象列表绑定的对象api名称。
- __deletedRecord: `Boolean`。是否为删除操作，若为真，则跳转到对象列表。选填。

在华炎魔方记录详情页中删除记录后，会跳转对象列表页面组件，通过在弹出的编辑表单中监听提交成功事件执行此动作来实现的。
...
相关列表监听了对象的`@data.change`事件，会刷新相关列表组件。详情请参考 [相关表组件源码](https://github.com/steedos/steedos-widgets/blob/1.2/packages/%40steedos-widgets/amis-lib/src/lib/converter/amis/fields/lookup.js#L419)。

以下示例演示点击按钮后，刷新名为`project`的相关列表组件。
...
相关列表组件会监听`@data.changed.project`事件，并根据传入的参数判断是否刷新相关列表组件。

要刷新相关列表组件，需在`data`中配置以下参数：
  * objectName: `String`。要触发广播事件对象的对象Api名称，即对象列表绑定的对象api名称。

在华炎魔方对象相关列表执行增、删、改后，会刷新页面。[相关列表广播源码](https://github.com/steedos/steedos-widgets/blob/1.2/packages/@steedos-widgets/amis-lib/src/lib/objectsRelated.js#L161)。  
