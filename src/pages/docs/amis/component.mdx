...
- [列表视图 ObjectListView](/docs/amis/object_listview)
...
所有的列表视图组件都监听了对象的`@data.change`事件，支持执行以下行为：
- 刷新列表视图组件
- 跳转到记录详细页
...
在华炎魔方记录详情页中删除记录后，会跳转列表视图页面页面，就是因为触发了点击事件并触发了页面中只读的对象表单组件的`@data.change`事件，在`@data.change`事件中会执行跳转动作。
