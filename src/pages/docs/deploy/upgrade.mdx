...
<alert type="tip">
虽然这里提供升级向导，但是非必要的话，强烈建议用最新版本的华炎魔方创建全新的项目，而不是升级老项目，特别是项目本身版本号跨度大，又希望保全之前已有数据库数据的情况下，不太建议做大升级。
...
<alert type="tip">
一般来说直接按上述操作方法即可升级项目内核版本，但是有时版本号跨度比较大时会有些额外操作步骤，请参考以下对应版本升级说明。
...
请参考文档 [安装部署](/docs/deploy)，根据您要升级的目标环境按相应的文档教程操作。

### 激活华炎魔方

从2.1版本开始，所有的私有部署都需要在云平台上先激活，请参考文档 [私有部署 - 激活华炎魔方](/docs/deploy/deploy-activate)。

激活华炎魔方过程中新建“我的魔方”时，如果希望项目升级内核后使用新的数据库，“魔方ID”属性请空着不要填写，云平台会随机生成新的华炎魔方ID值；如果想继续使用项目原来已有数据库，就要填写为已有项目的魔方ID值，不可以空着，后续升级过程强依赖这个华炎魔方ID值。

已有项目的魔方ID值可以打开“设置>公司设置>公司”界面，浏览器地址栏中最后一个字符串就是魔方ID值，比如`/app/admin/spaces/view/WE9mT9WktDHAjJDpC` 中取最后的`WE9mT9WktDHAjJDpC`。

\

<alert type="warning">
如果想保留项目原有数据库，一定要注意“魔方ID”属性不可以空着，否则云平台会随机生成新的华炎魔方ID值，这样升级后就无法正常使用原项目已有数据库。
...
<alert type="info">
因为云平台上每个账户暂时只能创建一个华炎魔方，所以如果目前想创建更多魔方ID来升级老项目的话，只能为每个老项目注册一个云账户，以后云平台有可能会允许一个账户创建多个魔方ID。
...
见 [project-template](https://github.com/steedos/steedos-platform/tree/2.1/steedos-projects/project-template)， 你也可以选择命令行创建一个2.1版本的华炎魔方项目，会自动下载并生成这个项目模板文件夹。
