...
actions: {
    ...
    spaceUsersBeforeUpdate: {
        trigger: { objectName: 'space_users', when: ['beforeInsert', 'beforeUpdate']},
        async handler(ctx) {
...
query | 查询数据相关参数[json], before.find时存在此属性
data | 查询结果, after.find时存在此属性
when | 触发时机。