...
* 示例1，点击整个表单任意位置时弹出新窗口，在表单的编辑和只读状态新窗口分别跳转到不同的网址。代码如下：
...
* 示例2，当表单加载完成后，根据任务优先级请求第三方API接口获取到期日期默认值。代码如下：
  ```yml
  # tasks.object.yml
  form:
    onRendered: !!js/function |
...
调用 bindFormEvent 或 bindFieldEvent 绑定原生事件时回调函数。
...
调用 bindFormEvent 或 bindFieldEvent 绑定原生事件时回调函数所带的参数。
...
field属性在bindFormEvent 或 bindFieldEvent函数中都表示触发事件时点击的字段Schema，不过bindFormEvent因为响应的是整个表单的事件，所以该属性值可能为空，而后者因为事件是直接绑定到字段上，所以肯定有值。

record表示修改记录时从接口中抓取到的记录内容，新建记录时该属性值为空。
