...
<alert type="info">
通过steedos cli命令行新建的华炎魔方项目会自带JsReport项目工程，您只要在命令行中运行`npx create-steedos-app@next -e jsreport-app`即可生成一个带有初始化好的JsReport项目工程的华炎魔方项目，不需要手动新建`jsreport-app`文件夹来初始化JsReport项目工程。
