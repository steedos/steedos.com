...
假设魔方项目目录在`/workspace/project-demo`，里面有一个名称为“项目管理”的软件包，其目录会类似以下结构：
...
我们接下来打开命令行在项目根目录，即`steedos-demo`下执行以下命令来生成名称为`project`的软件包文件夹下的各种类型元数据对应的翻译文件：
