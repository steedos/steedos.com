...
大家可以先在魔方项目中新建一个 [软件包](https://www.steedos.com/docs/developer/package) 专门用于日本语的国际化，假设我们新建了一个名为`locale-ja`的空软件包，其目录结构如下：
...
假设魔方项目目录在`/workspace/project-demo`，里面有一个名称为“项目管理”的软件包，其目录会类型以下结构：
...
我们接下来打开命令行在项目根目录，即`project-demo`下执行以下命令来生成名称为`project`的软件包文件夹下的各种类型元数据对应的翻译文件：

```bash
steedos i18n ja -s . -p /workspace/project-demo/steedos-packages/project/main
```
