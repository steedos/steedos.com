...
在添加好自定义翻译变量后，我们就可以在程序源码中使用它们了，以下示例演示了如何在触发器中抛出错误消息时使用上面新添加的自定义翻译变量。
...
从上述示例代码中可以看到，在触发器中只要在抛出错误信息时直接引用自定义翻译变量的key，就可以自动实现错误消息国际化了。
