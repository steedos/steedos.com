---
title: ObjectQL服务端语法说明
description: 在服务端实现操作数据库中的数据，包括增删改查、分页、排序等等。
---

使用ObjectQL语法，可以在服务端实现操作数据库中的数据，包括增删改查、分页、排序等等。
...
* query: `SteedosQueryOptions` 查询数据相关参数\[json\]。
  * *filters:* `SteedosQueryFilters`*查询条件数组, 选填*
* externalPipeline: `Array`[Mongodb ggregation Pipeline](https://docs.mongodb.com/manual/core/aggregation-pipeline/)
* userSession: `SteedosUserSession` 当前用户登录信息，非必填；如果未传入userSession，则不进行权限校验。
...
触发器详细说明文档见：[触发器](https://www.steedos.cn/docs/developer/getting-started#%E8%A7%A6%E5%8F%91%E5%99%A8)
